...
local plugin = {
	...
	unlock_buf = function(buf)
		vim.api.nvim_set_option_value("modifiable", true, { buf = buf })
	end,
	lock_buf = function(buf)
		vim.api.nvim_set_option_value("modifiable", false, { buf = buf })
	end,
	screen_height = function(window)
		return vim.api.nvim_win_get_height(window) - vim.opt.cmdheight:get()
	end,
	screen_width = function(window)
...
local function display_welcome(payload)
	...
	if not is_dir and default_buff_name ~= "" and default_buff_filetype ~= plugin.name then
		return
	end
