  ...
  for i, line in ipairs(lines) do
    if line:match "%S" then
      if all_commented then
        if post ~= "" and line:sub(-#post) == post then line = line:sub(1, #line - #post) end
...
local function map(mode, lhs, rhs, desc) vim.keymap.set(mode, lhs, rhs, { desc = desc or "comment", silent = true }) end
