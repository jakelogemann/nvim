...
n('<c-w>\\', function() local ok,w=pcall(require,'which-key'); if ok then w.show{ keys='<c-w>', loop=true } end end, 'window mode')
...
n('<leader>pf', function() require('telescope.builtin').find_files{ cwd = config_root, prompt_title='Config files'} end, 'Find config files')
n('<leader>pP', function() require('telescope.builtin').find_files{ cwd = config_root .. '/lua/custom/plugins', prompt_title='Plugin specs'} end, 'Find plugin specs')
n('<leader>pR', function() require('telescope.builtin').find_files{ prompt_title='Runtime files', search_dirs = vim.api.nvim_list_runtime_paths() } end, 'Find runtime files')
n('<leader>pL', function() require('telescope.builtin').live_grep{ prompt_title='Lua (config) grep', search_dirs={config_root .. '/lua'}, additional_args=function() return {'--glob','*.lua'} end } end, 'Grep Lua in config')
n('<leader>pS', function() require('telescope.builtin').live_grep{ prompt_title='Lua (runtime) grep', search_dirs = vim.tbl_map(function(p) return p .. '/lua' end, vim.api.nvim_list_runtime_paths()), additional_args=function() return {'--glob','*.lua'} end } end, 'Grep Lua runtime')
n('<leader>pM', function() local log = vim.fn.stdpath('cache') .. '/mason.log'; if vim.loop.fs_stat(log) then vim.cmd.edit(log) else vim.notify('No mason.log yet', vim.log.levels.INFO) end end, 'Open mason.log')
n('<leader>pc', '<cmd>Neoconf<cr>', 'project config')
n('<leader>ps', function() local ok,p=pcall(require,'persistence'); if ok then p.load() end end, 'session load')
n('<leader>pS', function() local ok,p=pcall(require,'persistence'); if ok then p.load{ last=true } end end, 'last session')
...
n('<leader>om', function() local ok,o=pcall(require,'ollama'); if ok and o.select_model then o.select_model() else vim.notify('No model selector','warn') end end, 'Model select')
...
n('<leader>Ca', function() local k=vim.api.nvim_replace_termcodes('<M-CR>',true,false,true); vim.api.nvim_feedkeys(k,'i',false) end, 'Copilot accept')
n('<leader>CT', function() if vim.g.copilot_enabled == 1 then vim.cmd('Copilot disable') else vim.cmd('Copilot enable') end end, 'Copilot toggle')

-- Rust
n('<leader>rt', function() require('custom.utils').run_in_term('cargo test') end, 'cargo test')
n('<leader>rf', function() local f=vim.fn.expand('%'); require('custom.utils').run_in_term('cargo test -- '..f) end, 'test file')
...
n('<leader>xx', function()
  ...
  local cmd
  if ft == 'go' then cmd = 'go run ' .. file
  elseif ft == 'rust' then cmd = 'cargo run'
  elseif ft == 'python' then cmd = 'python ' .. file
  elseif ft == 'sh' or ft == 'bash' or ft == 'zsh' or ft == 'fish' then cmd = file
  else cmd = file end
  local ok,u=pcall(require,'custom.utils'); if ok and u.run_in_term then u.run_in_term(cmd) else vim.cmd('!'..cmd) end
end, 'run file')

-- Editing utilities
n('<leader>yd', function() vim.cmd('t.') end, 'duplicate line')
n('<leader>sS', function() local w=vim.fn.expand('<cword>'); vim.cmd('%%s/\\<'..w..'\\>//gI') end, 'substitute word')
...
n('<leader>zw', function() vim.ui.input({prompt='set shiftwidth '}, function(input) local n=tonumber(input); if n then vim.o.shiftwidth=n end end) end, 'Set shiftwidth')
