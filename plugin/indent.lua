...
local function detect_indent(buf)
    ...
    if line and not is_comment_line(line) then
      ...
      if ws ~= '' then
        ...
        for ch in ws:gmatch('.') do
          if ch == ' ' then indent_len = indent_len + 1
          elseif ch == '\t' then
        ...
        if indent_len > 0 then
          if last_indent then
            ...
            if delta > 0 and delta <= 8 then
              deltas[delta] = (deltas[delta] or 0) + 1
            end
          end
  ...
  local expand = true
  if tab_indents > 0 and tab_indents >= space_indents * 2 then
    expand = false
  end
...
local function ensure_hl()
  ...
  if not ok then
    api.nvim_set_hl(0, CONFIG.guides.highlight, { link = 'NonText', default = true })
  end
end

local function current_shift()
...
local function visible_range(win)
  return fn.line('w0', win), fn.line('w$', win)
end
...
local function schedule()
  ...
  timer:start(CONFIG.guides.debounce_ms, 0, vim.schedule_wrap(function()
