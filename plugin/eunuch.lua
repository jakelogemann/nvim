...
api.nvim_create_user_command('Rename', function(opts)
  vim.cmd('Move ' .. table.concat(opts.fargs, ' '))
end, { nargs = 1, complete = 'file' })
...
api.nvim_create_user_command('Chmod', function(opts)
  ...
  if ok then notify('chmod ' .. mode .. ' ' .. path) else notify('chmod failed', vim.log.levels.ERROR) end
end, { nargs = 1 })
...
api.nvim_create_user_command('Mkdir', function(opts)
  ...
  if ok == 1 then notify('Created directory ' .. path) else notify('Failed to create directory', vim.log.levels.ERROR) end
...
api.nvim_create_user_command('Cfind', function(opts)
  ...
  fn.setqflist({}, ' ', { title = 'Cfind ' .. pattern, items = vim.tbl_map(function(f)
...
api.nvim_create_user_command('Lfind', function(opts)
  ...
  fn.setloclist(0, {}, ' ', { title = 'Lfind ' .. pattern, items = vim.tbl_map(function(f)
    return { filename = f, lnum = 1, col = 1, text = f }
  end, results) })
...
api.nvim_create_user_command('Clocate', function(opts)
  ...
  fn.setqflist({}, ' ', { title = 'Clocate ' .. pattern, items = vim.tbl_map(function(f)
    return { filename = f, lnum = 1, col = 1, text = f }
  end, results) })
...
api.nvim_create_user_command('Llocate', function(opts)
  ...
  if #results == 0 then return notify('No matches (locate)') end
  fn.setloclist(0, {}, ' ') -- clear
  fn.setloclist(0, {}, ' ', { title = 'Llocate ' .. pattern, items = vim.tbl_map(function(f)
    return { filename = f, lnum = 1, col = 1, text = f }
  end, results) })
