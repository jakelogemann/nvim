...
local function close_window(opts)
  ...
  if not opts.no_auto_close then
    if globals.float_win ~= nil then vim.api.nvim_win_hide(globals.float_win) end
    if globals.result_buffer ~= nil then vim.api.nvim_buf_delete(globals.result_buffer, { force = true }) end
    reset()
  end
end
...
local function get_window_options(opts)
  ...
  if version.major > 0 or version.minor >= 10 then result.hide = opts.hidden end
...
local function create_window(cmd, opts)
  ...
  if display_mode == "float" then
    if globals.result_buffer then vim.api.nvim_buf_delete(globals.result_buffer, { force = true }) end
...
M.exec = function(options)
  ...
  if globals.result_buffer ~= vim.fn.winbufnr(0) then
    ...
    if mode == "v" or mode == "V" then
      ...
      if globals.end_pos[3] > max_col then globals.end_pos[3] = vim.fn.col "'>" - 1 end -- in case of `V`, it would be maxcol instead
    else
  ...
  local function substitute_placeholders(input)
    ...
    if string.find(text, "%$register") then
      ...
      if not register or register:match "^%s*$" then error "Prompt uses $register but yank register is empty" end
  ...
  if type(prompt) == "function" then
    ...
    if type(prompt) ~= "string" or string.len(prompt) == 0 then return end
  end
...
M.run_command = function(cmd, opts)
  ...
  if opts.show_prompt then
    ...
    for i = 1, #lines do
      ...
      if i >= 3 then
        if #lines > i then table.insert(short_prompt, "...") end
        break
      end
    end
    local heading = "#"
...
local function select_prompt(cb)
  ...
  vim.ui.select(promptKeys, {
    prompt = "Prompt:",
    format_item = function(item) return table.concat(vim.split(item, "_"), " ") end,
  }, function(item) cb(item) end)
end

vim.api.nvim_create_user_command(M.command_name, function(arg)
  local mode
  if arg.range == 0 then
    mode = "n"
  else
  ...
  complete = function(ArgLead)
    local promptKeys = {}
    for key, _ in pairs(M.prompts) do
      if key:lower():match("^" .. ArgLead:lower()) then table.insert(promptKeys, key) end
    end
    table.sort(promptKeys)
    return promptKeys
  end,
