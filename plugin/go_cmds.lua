...
local run_in_term = (ok_u and utils.run_in_term) or function(cmd, opts)
  vim.cmd((opts and opts.cwd and ("lcd " .. fn.fnameescape(opts.cwd) .. " | ") or "") .. "!" .. cmd)
end
...
api.nvim_create_user_command("GoTest", function()
  run_in_term("go test ./...", { cwd = file_dir() })
end, { desc = "Run all tests in module" })

api.nvim_create_user_command("GoTestFile", function()
  run_in_term("go test -v .", { cwd = file_dir() })
end, { desc = "Run tests in current package" })
...
api.nvim_create_user_command("GoBuild", function()
  run_in_term("go build ./...", { cwd = file_dir() })
end, { desc = "Build packages" })

api.nvim_create_user_command("GoInstall", function()
  run_in_term("go install ./...", { cwd = file_dir() })
end, { desc = "Install module binaries" })
